...
class TestC64Startup:
    ...
    def test_c64_cpu_starts_at_reset_vector(self, c64_context: MiniRacer):
        ...
        assert (
            result["resetVector"] >= 0xE000
        ), "Reset vector should be in KERNAL ROM area"
    ...
    def test_c64_shows_ready_prompt(self, c64_context: MiniRacer):
        ...
        assert (
            result["cpuCycles"] > 100000
        ), f"CPU should have executed, only ran {result['cpuCycles']} cycles"

        assert result[
            "hasReady"
        ], f"Boot did not reach READY; PC=${result['cpuPC']:04X}, cycles={result['cpuCycles']}"
...
class TestSIDFilesPlayback:
    ...
    def test_sid_file_loads_and_plays(
        self, sid_player_context: MiniRacer, sid_name: str
    ):
        ...
        result = call_js_test_with_bytes(
            sid_player_context, "testC64MachineSidPlayback", sid_bytes, 10
        )

        assert (
            result.get("loaded") is True
        ), f"Failed to load {sid_name}: {result.get('error')}"
        assert result["totalSamples"] > 0, f"{sid_name}: No samples generated"
        assert (
            result["hasAudio"] is True
        ), f"{sid_name}: No audio produced (all samples are zero)"
        print(f"\n  {sid_name}: '{result['tuneName']}' by {result['tuneAuthor']}")
        print(
            f"    Tracks: {result['tuneSongs']}, Samples: {result['totalSamples']}, Max amplitude: {result['maxAmplitude']}"
        )

    def test_all_sid_files_in_directory(
        self, sid_player_context: MiniRacer, sid_files: list[Path]
    ):
...
class TestC64MachineSidPlayback:
    ...
    def test_c64machine_plays_sid_file(
        self, sid_player_context: MiniRacer, sid_name: str
    ):
        ...
        assert (
            result["cia1Running"] is True or result["vicIrqEnabled"] is True
        ...
        print(
            f"    Songs: {result['tuneSongs']}, Samples: {result['totalSamples']}, Non-zero: {result['nonZeroSamples']}, Max: {result['maxAmplitude']}"
        )
        print(
            f"    CPU PC: ${result['cpuPC']:04X}, CIA1 running: {result['cia1Running']}"
        )
    ...
    def test_c64machine_cia2_nmi_support(self, c64_context: MiniRacer):
        ...
        assert (
            result["nmiLineInitiallyHigh"] is True
        ), "NMI line should start high (inactive)"
        assert (
            result["nmiTriggered"] is True
        ), "NMI should have been triggered by timer underflow"
