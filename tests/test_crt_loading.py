...
class TestCartridgeParsing:
    ...
    def test_parse_river_raid_crt(
        self, crt_context: MiniRacer, river_raid_bytes: list[int]
    ):
        """Test parsing River Raid CRT (16K normal cartridge)."""
        # Load the CRT data
        crt_context.eval(
            f"var crtBytes = new Uint8Array({json.dumps(river_raid_bytes)});"
        )
    ...
    def test_parse_easyflash_crt(
        self, crt_context: MiniRacer, easyflash_bytes: list[int]
    ):
        """Test parsing EasyFlash CRT (type 32)."""
        crt_context.eval(
            f"var crtBytes = new Uint8Array({json.dumps(easyflash_bytes)});"
        )
...
class TestCartridgeMemoryMapping:
    ...
    def test_river_raid_memory_config(
        self, crt_context: MiniRacer, river_raid_bytes: list[int]
    ):
    ...
    def test_pitfall_memory_config(
        self, crt_context: MiniRacer, pitfall_bytes: list[int]
    ):
    ...
    def test_river_raid_roml_read(
        ...
        assert all(
            b is not None for b in bytes_read
        ), "Should read valid ROM data from ROML"
    ...
    def test_pitfall_romh_not_mapped(
        self, crt_context: MiniRacer, pitfall_bytes: list[int]
    ):
...
class TestC64MachineCartridge:
    ...
    def test_machine_load_cartridge(
        self, crt_context: MiniRacer, river_raid_bytes: list[int]
    ):
    ...
    def test_machine_read_cartridge_rom(
        self, crt_context: MiniRacer, river_raid_bytes: list[int]
    ):
    ...
    def test_machine_read_romh_in_16k_mode(
        self, crt_context: MiniRacer, river_raid_bytes: list[int]
    ):
...
class TestCartridgeAutostart:
    ...
    def test_river_raid_coldstart_signature(
        self, crt_context: MiniRacer, river_raid_bytes: list[int]
    ):
    ...
    def test_pitfall_coldstart_signature(
        self, crt_context: MiniRacer, pitfall_bytes: list[int]
    ):
...
class TestCartridge16KBHandling:
    ...
    def test_16kb_bank_maps_to_both_regions(
        self, crt_context: MiniRacer, river_raid_bytes: list[int]
    ):
...
class TestEasyFlashCartridge:
    ...
    def test_easyflash_parse_header(
        self, crt_context: MiniRacer, ghostsngoblins_bytes: list[int]
    ):
        """Test that EasyFlash cartridge header is parsed correctly."""
        crt_context.eval(
            f"var crtBytes = new Uint8Array({json.dumps(ghostsngoblins_bytes)});"
        )
        ...
        assert (
            info["numBanks"] == 64
        ), "Ghosts'n Goblins should have 64 banks (32 ROML + 32 ROMH)"

    def test_easyflash_ram_initialized(
        self, crt_context: MiniRacer, ghostsngoblins_bytes: list[int]
    ):
    ...
    def test_easyflash_ram_read_write(
        self, crt_context: MiniRacer, ghostsngoblins_bytes: list[int]
    ):
    ...
    def test_easyflash_bank_switching(
        self, crt_context: MiniRacer, ghostsngoblins_bytes: list[int]
    ):
        ...
        assert (
            info["bank63"] == 63
        ), "After write(0xDE00, 0xFF), bank should be 63 (masked to 6 bits)"

    def test_easyflash_control_register(
        self, crt_context: MiniRacer, ghostsngoblins_bytes: list[int]
    ):
        ...
        assert (
            info["initialExrom"] == 1
        ), "Initial EXROM should be 1 (Ultimax mode from header)"

        # Control register should be updated
        assert info["controlOff"] == 0x04, "Control register should be 0x04"
        assert (info["exromOff"], info["gameOff"]) == (1, 1)

        # Verify 8K mode setting
        assert info["control8K"] == 0x06, "Control register should be 0x06"
        assert (info["exrom8K"], info["game8K"]) == (0, 1)

    def test_easyflash_roml_romh_bank_structure(
        self, crt_context: MiniRacer, ghostsngoblins_bytes: list[int]
    ):
        ...
        assert (
            info["uniqueBankNumbers"] == 32
        ), "Should have 32 unique bank numbers (0-31)"

    def test_easyflash_reads_both_roml_and_romh(
        self, crt_context: MiniRacer, ghostsngoblins_bytes: list[int]
    ):
        ...
        assert (
            info["hasRomhData"] is True
        ), "Should be able to read ROMH data at $E000 (Ultimax mode)"
        assert info["ultimaxMode"] is True, "Cart should be in Ultimax mode"

    def test_easyflash_reset(
        self, crt_context: MiniRacer, ghostsngoblins_bytes: list[int]
    ):
        ...
        assert (
            info["ramAfterReset"] == 0
        ), "EasyFlash RAM is cleared on reset in this emulator"
